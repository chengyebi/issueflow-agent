...
## 数据集构建（v3：repo+category 分层 + 时间切分，防泄漏，ground-truth 独立）

**v1 / v2 已废弃**，移至 `datasets/deprecated/`。
**v3** 使用 `repo_category_stratified_time`：每个 (repo, category) bucket 内按时间切分，
较早 80% -> DEV、较新 20% -> TEST。

| split | 条数 | bug | feature | question | doc | SHA-256 |
|---|---|---|---|---|---|---|
| **DEV** | 2011 | 1529 | 269 | 132 | 81 | `94a0a81a...` |
| **TEST** | 506 | 384 | 68 | 33 | 21 | `9ad6e989...` |
...
- near-duplicate grouping：`title-token-jaccard`（阈值 0.6，写入 manifest），2415 groups，**cross_split_group_count = 0** ✅
  - **group_id 已持久化到 JSONL**（P1.7）：`group_id IS NULL = 0`（DEV 与 TEST 都验证）
  - DEV groups 1922 ∩ TEST groups 493 = 0 ✅
- **ground-truth label 独立**（P1.9）：`expected_label` 全部来自 `source_labels` 真实 label，
  验证 `expected_label in source_labels` = 0 违例；`ambiguous_concrete_label_count = 24`（已排除）
- DEV 时间范围 2015-05 → 2026-03；TEST 2019-01 → 2026-08
- **TEST 内容/指标冻结前未查看**
- 排除统计：无核心标签 3593、生命周期 351、ambiguous concrete label 24（共 3968）
